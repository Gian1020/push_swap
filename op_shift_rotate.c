...
/* Sposta il primo elemento dello stack in ultima posizione (rotazione verso l'alto).
...
{
	return (shift_rotate(l_stack_a, "ra"));
}
...
{
	return (shift_rotate(l_stack_b, "rb"));
}
...
int	rr(t_stack **l_stack_a, t_stack **l_stack_b)
{
	...
	flag = 0;
	flag += shift_rotate(l_stack_a, NULL);
	flag += shift_rotate(l_stack_b, NULL);
	if (flag == 2)
		write(1, "rr\n", 3);
	return (flag);
}
